(function(root, factory) {
	...
	if(has_canvas) {
		...
		var draw_shape = function(context, shape, coords, x_shift, y_shift) {
			...
			//Get URL of this area (Preview Link)
			var coords_1  = coords.toString();
			var area_href = '';
			$('#ImageMapContainer area').each(function(){
				var coords_2 = $(this).attr('coords');
				if(coords_1 == coords_2){
					area_href = $(this).attr('href');
				}
			});
			...
			if(shape == 'rect') {
				...
				var padding = $('.main_container .preview_container .paper').css('padding');
				padding 	= parseInt(padding.replace('px',''));

				$('#mapping_overlay').css({
					'top':coords[1] + y_shift+padding+'px',
					'left':coords[0] + x_shift+padding+'px',
					'width':coords[2] - coords[0]+'px',
					'height':coords[3] - coords[1]+'px'
				});
				$('#mapping_overlay').attr('href',area_href);
				$('#mapping_overlay').show();
